...
Missile::Missile(const QPointF &start_position, QColor color,
                 Player *const owner, bool direction_down, int time)
    : owner(owner), color(color), anim(this, "pos") {
...
QRectF Missile::boundingRect() const { return QRectF(0, 0, 1, 1); }
...
void Missile::paint(QPainter *painter, const QStyleOptionGraphicsItem *item,
                    QWidget *widget) {
  Q_UNUSED(item);
  Q_UNUSED(widget);
...
void Missile::advance(int phase) {
  if (phase == 0) {
    if (lookAround(owner)) remove();
    return;
  }
}

void Missile::hit(Player *) { remove(); }
