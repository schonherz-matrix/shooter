...
void Wreck::paint(QPainter *painter, const QStyleOptionGraphicsItem *item,
                  QWidget *widget) {
  Q_UNUSED(item);
  Q_UNUSED(widget);
...
QRectF Wreck::boundingRect() const { return QRectF(0, 0, 1, 1); }
...
void Wreck::advance(int phase) {
  ...
  if (this->pos().x() > 32 || this->pos().x() < 0 || this->pos().y() < 0 ||
      this->pos().y() > 26) {
